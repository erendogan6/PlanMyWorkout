package com.erendogan6.planmyworkout.domain.usecase.auth;

import androidx.lifecycle.LiveData;

import com.erendogan6.planmyworkout.domain.model.Result;
import com.erendogan6.planmyworkout.domain.repository.AuthRepository;
import com.erendogan6.planmyworkout.domain.usecase.base.LiveDataUseCase;

import javax.inject.Inject;

/**
 * Use case for logging in a user.
 * This class is part of the domain layer and encapsulates the business logic for user login.
 */
public class LoginUseCase implements LiveDataUseCase<LoginParams, Result<Boolean>> {
    
    private final AuthRepository authRepository;
    
    @Inject
    public LoginUseCase(AuthRepository authRepository) {
        this.authRepository = authRepository;
    }
    
    @Override
    public LiveData<Result<Boolean>> execute(LoginParams params) {
        return authRepository.login(params.getEmail(), params.getPassword());
    }
}
